      ...
      <View
        ...
        <View className="flex-row items-center">
          ...
          <View className="flex-1">
            <ThemedText testID={`uploaded-file-name-${index}`} className="text-lg font-bold mb-1">
              {file.name}
            </ThemedText>
            <ThemedText testID={`uploaded-file-meta-${index}`} className="text-sm text-gray-600 dark:text-gray-400 mb-1">
    ...
    <ThemedView className="flex-1">
      <ScrollView className="flex-1 p-6">
        ...
        <View className="mb-8">
          ...
          <View className="flex-row items-center">
            <View className="flex-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-3 mr-3">
              <ThemedText testID="selected-file-name" className="text-gray-500 dark:text-gray-400">
                {selectedFileName || 'No file selected'}
              </ThemedText>
            </View>
            <Pressable
              testID="browse-file-button"
              accessibilityLabel="Browse for a file to upload"
        ...
        {Object.keys(uploads).some(fileId => uploads[fileId].progress < 1) && (
          <View testID="upload-progress" className="bg-white dark:bg-gray-800 rounded-2xl p-6 mb-6 border-2 border-gray-100 dark:border-gray-700">
              ...
              .map(([fileId, upload]) => (
                <View key={fileId} className="mb-4">
                  <View className="flex-row justify-between items-center mb-2">
                    <ThemedText testID="upload-progress-status" className="text-sm font-medium">
                      {upload.uploading ? 'Uploading...' : 'Complete'}
                    </ThemedText>
                    <ThemedText testID="upload-progress-percent" className="text-sm text-gray-600 dark:text-gray-400">
        ...
        {uploadedFiles.length === 0 && Object.keys(uploads).length === 0 && (
          <View className="items-center py-12">
            ...
            <ThemedText testID="no-files-message" className="text-gray-500 dark:text-gray-400 text-center mt-4">
